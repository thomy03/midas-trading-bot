...
class FundamentalPillar(BasePillar):
    ...
    def __init__(self, weight: float = 0.25):
        ...
        # Sector average P/E ratios - load from cache if available, else use defaults
        _default_sector_pe = {
            'Technology': 30,
            'Healthcare': 25,
        ...
        # V8.1: Try to load dynamic sector P/E from cache
        import os, json as _json
        _cache_path = os.path.join(os.environ.get('MIDAS_DATA_DIR', '/app/data'), 'sector_fundamentals_cache.json')
        if os.path.exists(_cache_path):
            try:
                with open(_cache_path, 'r') as _f:
                    _cached = _json.load(_f)
                # Merge: cache values override defaults
                self.sector_pe = {**_default_sector_pe, **{k: v.get('pe_ratio', _default_sector_pe.get(k, 20)) if isinstance(v, dict) else v for k, v in _cached.items()}}
                if 'default' not in self.sector_pe:
                    self.sector_pe['default'] = 20
            except Exception:
                self.sector_pe = _default_sector_pe
        else:
            self.sector_pe = _default_sector_pe
