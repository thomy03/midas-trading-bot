#!/usr/bin/env python3
"""
MIDAS Backtest - OPTIMIZED VERSION
===================================
Objectif: Battre SPY avec Max DD < QQQ

Optimisations:
1. Max hold: 30 → 45 jours (capturer plus de momentum)
2. Stop loss: 2.0 → 2.5 ATR (moins de faux stops)
3. Trailing stop: 10% → 15% (plus de marge)
4. Take profit: 25% → 35% (laisser courir les gagnants)
5. Signal threshold: 50 → 55 (trades de meilleure qualité)
6. Regime filter: Réduire exposition en BEAR
7. Position sizing: Basé sur score (plus de conviction = plus de capital)
"""
...
def score_technical(idx: int, df: pd.DataFrame) -> float:
    ...
    # RSI (+/- 10) - OPTIMIZED: prefer 40-60 range
    if 40 <= rsi <= 55:
        score += 10  # Sweet spot
    elif 30 <= rsi < 40:
        score += 8
...
def backtest_symbol(
    ...
    signal_threshold: float = 55,  # OPTIMIZED: Higher threshold
    stop_loss_atr_mult: float = 2.5,  # OPTIMIZED: Wider stop
    take_profit_pct: float = 0.35,  # OPTIMIZED: Higher target
    max_hold_days: int = 45,  # OPTIMIZED: Longer hold
    trailing_stop_pct: float = 0.15,  # OPTIMIZED: Wider trailing
    regime_filter: bool = True  # NEW: Regime filter
) -> List[Trade]:
    trades = []
    position = None
    ...
    for i in range(200, len(df)):
        ...
        if position is None:
            ...
            # REGIME FILTER: Skip entries in BEAR market
            if regime_filter and regime == 'BEAR':
                continue
            
            # REGIME FILTER: Reduce position size in VOLATILE
            position_mult = 0.7 if regime == 'VOLATILE' else 1.0
            
            # SCORE-BASED SIZING: Higher score = larger position
            if total_score >= 65:
                position_mult *= 1.2  # 20% larger for high conviction
            elif total_score >= signal_threshold:
                position_mult *= 1.0
            else:
                continue  # Don't enter
            
            shares = (capital * position_mult) / close
            if shares > 0 and total_score >= signal_threshold:
                ...
                capital = max(0, capital - close * shares)
        else:
...
def run_backtest(symbols: List[str], start_date: str, end_date: str, capital_per_symbol: float = 1000):
    print("=" * 70)
    print("MIDAS BACKTEST - OPTIMIZED (v2)")
    print("=" * 70)
    print(f"\nPeriod: {start_date} to {end_date}")
    print(f"Symbols: {len(symbols)}")
    print(f"\nOPTIMIZATIONS APPLIED:")
    print(f"  ✓ Signal threshold: 50 → 55")
    print(f"  ✓ Stop loss: 2.0 → 2.5 ATR")
    print(f"  ✓ Take profit: 25% → 35%")
    print(f"  ✓ Max hold: 30 → 45 days")
    print(f"  ✓ Trailing stop: 10% → 15%")
    print(f"  ✓ BEAR regime filter: ON")
    print(f"  ✓ Score-based position sizing: ON")
    ...
    print(f"  {'Metric':<20} {'MIDAS OPT':>12} {'SPY':>12} {'QQQ':>12}")
    print(f"  {'-'*56}")
    print(f"  {'Total Return':<20} {total_return:>+11.1f}% {'~85%':>12} {'~120%':>12}")
    print(f"  {'CAGR':<20} {cagr:>+11.1f}% {'~13%':>12} {'~17%':>12}")
    print(f"  {'Max Drawdown':<20} {max_dd:>11.1f}% {'~34%':>12} {'~35%':>12}")
    ...
    for t in sorted(all_trades, key=lambda x: x.pnl, reverse=True)[:5]:
        print(f"  {t.symbol}: {t.entry_date} → {t.exit_date} | {t.pnl_pct:+.1f}% | {t.regime}")
    ...
    with open('midas_optimized_backtest.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    print(f"\n✅ Results saved to midas_optimized_backtest.json")
    print("=" * 70)
