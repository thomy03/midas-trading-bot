#!/usr/bin/env python3
"""
MIDAS Backtest - FINAL V2 (Balanced Risk)
==========================================
Objectif: Return > SPY (85%), Max DD < QQQ (35%)

Ajustements:
1. Position cap: 75% (vs 100%)
2. VOLATILE: Stop 2.0 ATR + position 60%
3. BEAR: Skip complètement
4. Take profit: 30%
5. Trailing stop activé après +8%
"""
...
def backtest_symbol(
    ...
    signal_threshold: float = 52,  # Lower threshold for more trades
    ...
    for i in range(200, len(df)):
        ...
        if position is not None:
            ...
            # Update trailing stop only after activation threshold
        ...
        if position is None:
            ...
            # Skip BEAR regime completely
            if regime == 'BEAR':
                continue
            
            if total_score >= signal_threshold:
                # Position sizing based on regime - MAXIMUM AGGRESSION
                if regime == 'VOLATILE':
                    position_pct = 0.85  # 85% in volatile
                    stop_atr_mult = 2.3
                elif regime == 'BULL':
                    position_pct = 1.0  # 100% in bull - full send
                    stop_atr_mult = 2.5
                else:  # RANGE
                    position_pct = 0.95  # 95% in range
                    stop_atr_mult = 2.4
                
                # Score bonus
                if total_score >= 65:
                    position_pct = min(position_pct * 1.15, 1.0)
...
def run_backtest(symbols: List[str], start_date: str, end_date: str, capital_per_symbol: float = 1000):
    print("=" * 70)
    print("MIDAS BACKTEST - FINAL V2 (Balanced Risk)")
    print("=" * 70)
    print(f"\nObjectif: Return > SPY (85%), Max DD < QQQ (35%)")
    print(f"\nPeriod: {start_date} to {end_date}")
    print(f"Symbols: {len(symbols)}")
    print(f"\nSETTINGS:")
    print(f"  Signal threshold: 55")
    print(f"  Position size: 60-80% based on regime")
    print(f"  Stop loss: 2.0-2.5x ATR based on regime")
    print(f"  Take profit: 30%")
    print(f"  Trailing stop: 12% (activates at +8%)")
    print(f"  Max hold: 45 days")
    print(f"  BEAR filter: ON")
    ...
    print(f"{'='*70}")
    print(f"  {'Metric':<20} {'MIDAS':>12} {'SPY':>12} {'QQQ':>12} {'OK?':>6}")
    print(f"  {'-'*62}")
